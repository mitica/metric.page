...
export const bmiCalculator: ConverterConfig = {
  ...
  calculate: (inputs) => {
    ...
    let category = "converter_bmi_normal";
    ...
    return [
      { labelKey: "converter_bmi_result_label", value: Math.round(bmi * 10) / 10 },
...
export const bmrCalculator: ConverterConfig = {
  ...
  inputs: [
    ...
    { id: "weight", type: "number", labelKey: "converter_bmi_weight_label", min: 1, max: 300, step: 0.1, defaultValue: 70, unit: "kg" },
    { id: "height", type: "number", labelKey: "converter_bmi_height_label", min: 50, max: 250, step: 1, defaultValue: 175, unit: "cm" },
    { id: "age", type: "number", labelKey: "converter_common_age", min: 1, max: 120, step: 1, defaultValue: 30 },
  ],
  calculate: (inputs) => {
    const gender = String(inputs.gender || "male");
    const weight = getWeight(inputs);
    ...
    return [{ labelKey: "converter_bmr_result_label", value: Math.round(bmr), unit: "common_kcal_day" }];
  },
};
...
export const tdeeCalculator: ConverterConfig = {
  ...
  calculate: (inputs) => {
    ...
    return [
      { labelKey: "converter_tdee_result_label", value: Math.round(tdee), unit: "common_kcal_day" },
      { labelKey: "converter_tdee_bmr_label", value: Math.round(bmr), unit: "common_kcal_day" },
    ];
  },
};
...
export const pregnancyDueDate: ConverterConfig = {
  ...
  inputs: [
    { id: "lastPeriod", type: "date", labelKey: "converter_pregnancy_last_period", defaultValue: "" },
  ],
  calculate: (inputs) => {
    const dateStr = String(inputs.lastPeriod || "");
    if (!dateStr) return [{ labelKey: "converter_pregnancy_due_date", value: "-" }];
    const lmp = new Date(dateStr);
    if (isNaN(lmp.getTime())) return [{ labelKey: "converter_pregnancy_due_date", value: "-" }];
    const dueDate = new Date(lmp);
    dueDate.setDate(dueDate.getDate() + 280);
    const now = new Date();
    const diffDays = Math.floor((dueDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
    const weeksPregnant = Math.floor((now.getTime() - lmp.getTime()) / (1000 * 60 * 60 * 24 * 7));
    return [
      { labelKey: "converter_pregnancy_due_date", value: dueDate.toISOString().split("T")[0] },
      { labelKey: "converter_pregnancy_weeks_pregnant", value: Math.max(0, weeksPregnant) },
      { labelKey: "converter_pregnancy_days_remaining", value: Math.max(0, diffDays) },
    ];
  },
};
...
export const bacCalculator: ConverterConfig = {
  ...
  calculate: (inputs) => {
    ...
    const bac = Math.max(0, (alcoholGrams / (weight * 1000 * r)) * 100 - 0.015 * hours);
...
export const heartRateZones: ConverterConfig = {
  ...
  inputs: [
    { id: "age", type: "number", labelKey: "converter_common_age", min: 10, max: 100, step: 1, defaultValue: 30 },
  ],
  calculate: (inputs) => {
...
export const macroCalculator: ConverterConfig = {
  ...
  inputs: [
    { id: "calories", type: "number", labelKey: "converter_macro_calories_input", min: 500, max: 10000, step: 50, defaultValue: 2000, unit: "kcal" },
    { id: "goal", type: "select", labelKey: "converter_macro_goal", defaultValue: "balanced", options: [
...
export const healthConverters = [bmiCalculator, bodyFatCalculator, idealWeight, bmrCalculator, tdeeCalculator, pregnancyDueDate, bacCalculator, waterIntake, heartRateZones, macroCalculator];
